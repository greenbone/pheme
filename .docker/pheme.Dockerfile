...
EXPOSE 8000
USER 1001:1001
CMD ["uwsgi", "--need-plugin", "python3", "--module=pheme.wsgi:application", "--env", "DJANGO_SETTINGS_MODULE=pheme.settings", "--master", "--http=0.0.0.0:8000", "--processes=3", "--max-requests=100", "--enable-threads", "--vacuum"]
